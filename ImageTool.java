...
//	this is enough for the homework
	...
	for(int col = 0; col < 5; col++) {
		System.out.print("the " + col + " pixel is [0] " + "[" + col + "] ");
		
		for(int intensity = 0; intensity < 4; intensity++) {
			System.out.print("[" + pixels[0][col][intensity] + "] ");
		}
		System.out.println(" ");
	}

	 
// and now let us try something funny
/*	 ImageTool imTool = new ImageTool ();
		Image image = imTool.readImageFile ("/Users/kevin/eclipse-workspace/assignment1/src/assignment1/try.JPG");
		int[][][] pixels = imTool.imageToPixels (image);	
		...
		for(int row = 0; row < pixels.length; row++) { 
			...
			for(int col = 0; col < pixels[row].length; col++) {
				...
				for(int intensity = 0; intensity < pixels[row][col].length; intensity++) {
					System.out.print("[" + pixels[0][col][intensity] + "] ");
				}
				System.out.println(" ");
			}
...
/*	for (int row=0; row < pixels.length; row+=2) {
	...
	imTool.showImage (image2, "AFTER");
     ...
     int size = 500;
     int[][][] pixels = new int [size][size][4];
